...
number shown in the context (e.g., [1], [2], [3]). Multiple sources for the same fact: [1][3]. \
Do NOT group all citations at the end. Place each citation RIGHT AFTER the specific \
information it supports. Example: "Wind turbines typically have 3 blades [2] and can \
...
- TRÍCH DẪN INLINE: Khi sử dụng thông tin từ ngữ cảnh, bạn PHẢI trích dẫn INLINE ngay \
sau câu hoặc mệnh đề liên quan bằng [N] trong đó N là số Source hiển thị trong ngữ cảnh \
(ví dụ: [1], [2], [3]). Nhiều nguồn cho cùng thông tin: [1][3]. KHÔNG gom tất cả trích dẫn \
cuối câu trả lời. Đặt mỗi trích dẫn NGAY SAU thông tin mà nó hỗ trợ. \
