...
| Giới hạn | Mô tả |
|---|---|
| Phụ thuộc OpenAI API | Chatbot yêu cầu kết nối internet và OpenAI API hoạt động bình thường |
| Không hỗ trợ offline | Không thể sử dụng khi mất kết nối internet |
| Không tìm kiếm web | Chỉ trả lời dựa trên tài liệu đã nạp, không tìm kiếm internet |
| Chất lượng OCR | OCR cho PDF scan tiếng Việt có thể không chính xác 100% với tài liệu viết tay hoặc scan chất lượng thấp |
| Giới hạn context | Mỗi phiên chat lưu tối đa 40 tin nhắn (8000 tokens) trong bộ nhớ |
| Không real-time data | Không cập nhật dữ liệu tự động — cần nạp tài liệu mới thủ công |

### Hiệu năng hiện tại

| Chỉ số | Giá trị |
|---|---|
| Thời gian đến token đầu tiên (TTFT) | ~1-3 giây (phụ thuộc OpenAI API) |
| Streaming response | Giảm thời gian chờ cảm nhận đáng kể |
| Test pass rate | 85% (37/46 test cases) |
| Consistency | 100% (cùng câu hỏi cho cùng câu trả lời) |
| Bilingual accuracy | 100% (tự động phản hồi đúng ngôn ngữ) |
